...
def _filter_actions_legacy(actions: np.ndarray) -> np.ndarray:
...
def _index_single_traj_text_legacy(traj_name: Path,
                                   root: Path,
                                   caption_type: str,
                                   context_length: int,
                                   action_length: int,
    ...
    traj = EgoWalkTrajectory.from_dataset(name=traj_name,
                                          data_path=root)
    text_df = pd.read_parquet(root / "legacy" / "annotations" / caption_type / f"{traj_name}__annotations_{caption_type}.parquet")
    ...
    for _, row in text_df.iterrows():
        ...
        for i in range(0, n_window_steps * window_step, window_step):
            ...
            if np.isnan(actions).any():
                actions = _filter_actions_legacy(actions)
...
def index_gnm_text_legacy(caption_type: str,
                          context_length: int,
                          action_length: int,
                          context_step: int = 1,
                          action_step: int = 1,
    ...
    task_fn = partial(_index_single_traj_text_legacy,
                      caption_type=caption_type,
                      root=root,
                      context_length=context_length,
                      action_length=action_length,
